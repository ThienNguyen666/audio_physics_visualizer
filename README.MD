...
## 📂 3. Cấu trúc Dự án (Project Architecture)

Dự án được phân tách rõ ràng giữa Core Engine (High Performance) và UI/UX Frontend, đảm bảo khả năng mở rộng và dễ dàng bảo trì:

```text
audio-physics-visualizer/
├── physics-core/                 # 🦀 Lõi Engine Vật lý viết bằng Rust
│   ├── src/
│   │   └── lib.rs                # Thuật toán Spatial Hash Grid, tính toán va chạm 60k hạt & Vortex
│   ├── Cargo.lock                # Khóa phiên bản dependencies Rust
│   └── Cargo.toml                # Cấu hình package Rust & WebAssembly
├── public/                       # Chứa các file tĩnh (favicon, K-391 - Earth.mp3, ảnh demo...)
├── src/
│   ├── assets/                   # Tài nguyên cục bộ (hero images, svgs)
│   ├── components/
│   │   ├── CanvasEngine/
│   │   │   └── PhysicsCanvas.jsx # 🌌 WebGL/Three.js Renderer: Đọc trực tiếp Shared Memory từ WASM
│   │   ├── Common/
│   │   │   └── Toast.jsx         # Hệ thống thông báo trạng thái/lỗi UI
│   │   ├── Controls/
│   │   │   └── AudioPlayer.jsx   # 🎛️ Unified Control Center: Trình phát nhạc, điều khiển 60k hạt, Neon, Debug
│   │   └── Layout.jsx            # Điều phối tầng giao diện (UI Overlay vs Canvas Background)
│   ├── core/
│   │   └── audio/
│   │       └── AudioAnalyzer.js  # Bóc tách tần số âm thanh (Bass, Mid, Treble) qua Web Audio API
│   ├── hooks/
│   │   ├── useAudioStream.js     # Quản lý vòng đời AudioContext & Microphone
│   │   ├── useGameLoop.js        # Điều phối vòng lặp render đồng bộ (requestAnimationFrame)
│   │   └── useGlobalError.js     # Bắt và xử lý các ngoại lệ toàn cục
│   ├── App.jsx                   # Component gốc điều phối State (Theme, Hạt, Hiệu ứng)
│   ├── index.css                 # CSS toàn cục (Khử cuộn màn hình, cấu hình biến màu)
│   └── main.jsx                  # Điểm khởi chạy của React DOM
├── Dockerfile                    # Cấu hình đóng gói Multi-stage (Node.js Build -> Nginx Serve)
├── package.json                  # Quản lý thư viện Node.js (Three.js, React...)
└── vite.config.js                # Cấu hình Vite & plugin wasm-pack
```
